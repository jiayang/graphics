...
"""
ARG_COMMANDS = [ 'box', 'sphere', 'torus', 'circle', 'bezier', 'hermite', 'line', 'scale','triangle', 'move', 'rotate', 'save' ]
...
def parse_file( fname, edges, polygons, csystems, screen, zbuffer, color ):
    ...
    while c < len(lines):
        ...
        if line == 'push':
            ...
            add_polygon(polygons,int(args[0]),int(args[1]),int(args[2]),int(args[3]),int(args[4]),int(args[5]),int(args[6]),int(args[7]),int(args[8]))
            matrix_mult(csystems[-1], polygons)
            draw_polygons(polygons, screen, zbuffer, color)
            polygons = []
        elif line == 'torus':
