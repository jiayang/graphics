...
def scanline_convert(polygons, i, screen, zbuffer ):
    global colour
    colors = [[123,3,41],[33,143,2],[87,231,123],[255,0,0],[100,8,231],[255,0,255]]
    color = colors[colour]
    colour += 1
    colour = colour % len(colors)
    lst = polygons[i:i+3]
    def get_y(v):
        return v[1]
    lst.sort(key=get_y)
    ...
    x,y,z,m = bottom
    x1,z1 = float(bottom[0]),float(bottom[2])
    ...
    while y <= middle[1]:
        ...
        z += dz/dy
    x1,y,z1,m = middle
    x = bottom[0] + (dx/dy) * (middle[1] - bottom[1])
    z = bottom[2] + (dz/dy) * (middle[1] - bottom[1])
    while y <= top[1]:
        draw_line(int(x),int(y),z,int(x1),int(y),z1,screen,zbuffer,color)
        if top[1] - middle[1] == 0:
            x1 += (top[0] - middle[0])
